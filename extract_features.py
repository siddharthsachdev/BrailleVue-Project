...
def video_to_frames(video):
    path = config.test_path +  '/temporary_images'
    print(path)
    if os.path.exists(path):
        shutil.rmtree(path)
    os.makedirs(path)
    # video_path = config.test_path +  '/video/' +  video
    print(video)
    count = 0
    image_list = []
    # Path to video file
    ...
    while cap.isOpened():
        ...
        if ret is False:
            break
        cv2.imwrite(config.test_path + '/temporary_images/' +'frame' + str(count) + '.jpg', frame)
        image_list.append(config.test_path + '/temporary_images/' +'frame' + str(count) + '.jpg')
        count += 1
...
def extract_features(video, model):
    ...
    shutil.rmtree(config.test_path + '/temporary_images')
    return img_feats
